...
func (u *ui) onGeneratePost(r *http.Request) string {
	...
	stepGetPostDetails.SetHandler(u.stepHandlerGetPostDetails)

	stepGeneratePost := wf.NewStep()
	stepGeneratePost.SetName("Generate Post Content")
	stepGeneratePost.SetHandler(u.stepHandlerGeneratePost)

	stepSavePost := wf.NewStep()
	stepSavePost.SetName("Save Blog Post")
	stepSavePost.SetHandler(u.stepHandlerSavePost)
	...
	return hb.Swal(hb.SwalOptions{
		...
		RedirectURL:      shared.NewLinksFromRequest(r).AiPostEditor(map[string]string{"id": recordPostID}),
		RedirectSeconds:  3,
	}).ToHTML()
}

func (u *ui) stepHandlerGetPostDetails(ctx context.Context, data map[string]any) (context.Context, map[string]any, error) {
...
func (u *ui) stepHandlerGeneratePost(ctx context.Context, data map[string]any) (context.Context, map[string]any, error) {
	recordPostID, ok := data["record_post_id"].(string)
	if !ok || recordPostID == "" {
		return ctx, data, errors.New("record_post_id is missing from pipeline data")
	}
	...
	blogAgent := blogai.NewBlogWriterAgent(u.Logger())
	if blogAgent == nil {
...
func (u *ui) stepHandlerSavePost(ctx context.Context, data map[string]any) (context.Context, map[string]any, error) {
	recordPostID, ok := data["record_post_id"].(string)
	if !ok || recordPostID == "" {
		return ctx, data, errors.New("record_post_id is missing from pipeline data")
	}

...
// postMediaComponent handles the media tab of the post update view.
// It embeds the parent *ui to access Store(), Logger(), and the
// fileManagerURL for the "Add from file manager" flow.
type postMediaComponent struct {
	ui *ui
}

func (u *ui) newPostMediaComponent() *postMediaComponent {
	return &postMediaComponent{ui: u}
}
...
func (c *postMediaComponent) Render(r *http.Request, post blogstore.PostInterface) hb.TagInterface {
	...
	if err != nil {
		c.ui.Logger().Error("Failed to read post media HTML template", "error", err)
		return hb.Div().HTML("Error loading media component")
	}

	jsContent, err := postCategoriesFiles.ReadFile("post_media.js")
	if err != nil {
		c.ui.Logger().Error("Failed to read post media JavaScript file", "error", err)
		return hb.Div().HTML("Error loading media component")
	}
	...
	initScript := `
		...
		const urlMediaUpload = '` + linksHelper.PostUpdate(map[string]string{"post_id": post.GetID(), "action": "upload-media"}) + `';
		const urlMediaSave = '` + linksHelper.PostUpdate(map[string]string{"post_id": post.GetID(), "action": "save-media"}) + `';
		const urlMediaDelete = '` + linksHelper.PostUpdate(map[string]string{"action": "delete-media"}) + `';
		const urlMediaAdd = '` + linksHelper.PostUpdate(map[string]string{"post_id": post.GetID(), "action": "add-media"}) + `';
	`
...
func (c *postMediaComponent) HandleLoad(w http.ResponseWriter, r *http.Request) string {
	...
	files, err := c.ui.Store().MediaListByEntityID(r.Context(), postID)
	if err != nil {
		c.ui.Logger().Error("Failed to load post files", "error", err)
...
func (c *postMediaComponent) HandleUpload(w http.ResponseWriter, r *http.Request) string {
	...
	if err := r.ParseMultipartForm(50 << 20); err != nil { //#nosec G120 -- 50MB limit is explicit, not unbounded
	...
	existingFiles, _ := c.ui.Store().MediaListByEntityID(r.Context(), postID)
	...
	for i, fileHeader := range files {
		...
		if err := c.ui.Store().MediaCreate(r.Context(), postFile); err != nil {
...
func (c *postMediaComponent) HandleSave(w http.ResponseWriter, r *http.Request) string {
	...
	for _, item := range reqData.Files {
		file, err := c.ui.Store().MediaFindByID(r.Context(), item.ID)
		if err != nil || file == nil {
			continue
		}
		if item.Name != "" {
		...
		if err := c.ui.Store().MediaUpdate(r.Context(), file); err != nil {
			c.ui.Logger().Error("Failed to update post file", "error", err, "file_id", item.ID)
		}
	}
...
func (c *postMediaComponent) HandleDelete(w http.ResponseWriter, r *http.Request) string {
	...
	file, err := c.ui.Store().MediaFindByID(r.Context(), fileID)
	if err != nil {
	...
	if err := c.ui.Store().MediaDeleteByID(r.Context(), fileID); err != nil {
